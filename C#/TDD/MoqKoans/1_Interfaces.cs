...
namespace MoqKoans
{
	...
	public class Moq1_Interfaces : Koan
	{
		...
		{
			var volumeMock = new Moq.Mock<IVolume>();
			Assert.AreEqual(false, volumeMock is IVolume);
			Assert.IsTrue(volumeMock is Moq.Mock<IVolume>);
		}
		...
		{
			var volumeMock = new Moq.Mock<IVolume>();
			Assert.AreEqual(true, volumeMock.Object is IVolume);
		}
