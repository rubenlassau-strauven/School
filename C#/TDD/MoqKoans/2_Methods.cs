...
namespace MoqKoans
{
	...
	public class Moq2_Methods : Koan
	{
		...
		{
			var mock = new Mock<IVolume>();
			mock.Setup(m => m.Quieter(It.Is<int>(n => n < 0))).Returns(0);
			mock.Setup(m => m.Quieter(It.Is<int>(n => n > 0))).Returns(100);
		...
		public void CreateAMockIAdditionThatOnlyAddsPositiveNumbersAndThrowsAnExceptionIfEitherNumerIsNegative()
		{
			...
			mock.Setup(x => x.Add(It.Is<int>(n => n >= 0), It.Is<int>(n => n >= 0))).Returns<int,int>((left,right) => left + right);
